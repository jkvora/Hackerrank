...
int dfs (int k) {
    ...
    while (sp > 0) {
        ...
        while (Last[k] > 0 && (was[t[Last[k]]] == iteration || cut[t[Last[k]]])) Last[k] = p[Last[k]];
        if (Last[k] > 0) {
            st[++sp] = t[Last[k]];
            Last[k] = p[Last[k]];
            continue;
        }
        if (sp > 1) subtree[st[sp - 1]] += subtree[k];
        --sp;
    }
    return subtree[st[1]];
}
...
void centroid_build (int k) {
    ...
    for(i = 1; i <= sn_sz; i++) {
        ...
        q = f[tn];
        while (q > 0) {
            if (was[t[q]] == iteration && subtree[tn] > subtree[t[q]]) max_subtree = max(max_subtree, subtree[t[q]]);
            q = p[q];
        }
...
void add_vertex (int k) {
    for(j = 0; j < belongs[k].size(); j++) {
        ...
        } else ret += sum_of_distances[cur.cut_vertex];
    }
}
...
void modify (int pos, int l, int r, int j) {
    if (tree[pos].l == l && tree[pos].r == r) tree[pos].tag = j; else {
        if (l <= min(r, tree[pos + pos].r)) modify(pos + pos, l, min(r, tree[pos + pos].r), j);
        if (max(l, tree[pos + pos + 1].l) <= r) modify(pos + pos + 1, max(l, tree[pos + pos + 1].l), r, j);
    }
}

int get_node (int pos, int j) {
    if (tree[pos].l == tree[pos].r) return tree[pos].tag; else {
        if (j <= tree[pos + pos].r) return max(tree[pos].tag, get_node(pos + pos, j)); else
...
int main (int argc, char * const argv[]) {
    ...
    for(i = 2; i <= n; i++) {
        ...
        if (bst_left[i] <= bst_right[i]) modify(1, bst_left[i], bst_right[i], i);
    }   
    centroid_build(1);
    for(i = 1; i <= n; i++) {
        add_vertex(i);
